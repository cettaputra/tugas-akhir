...
def speech():
    ...
    # Konversi angka ke kata (untuk label di TTS)
    digit_to_word = {
        '0': 'nol', '1': 'satu', '2': 'dua', '3': 'tiga', '4': 'empat',
        '5': 'lima', '6': 'enam', '7': 'tujuh', '8': 'delapan', '9': 'sembilan'
    }
    spoken_label = digit_to_word[label] if label in digit_to_word else label

    # Buat TTS
    tts = gTTS(f"Gambar ini adalah {spoken_label}. Yuk, kita ucapkan: {spoken_label}", lang='id')
    audio_path = os.path.join("static", "tts_speech.mp3")
    tts.save(audio_path)
    timestamp = datetime.now().timestamp()
    audio_url = f"/static/tts_speech.mp3?v={timestamp}"

    return render_template('speech.html', image=image, label=spoken_label, audio_url=audio_url)
